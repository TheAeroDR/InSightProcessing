...
def SolPath(Sol):
    ...
    eng_path = './sc_eng_data_LTST/'
    seis_path = './seis_data_calibrated_LTST/'
...
def data_extract(temp, sol):
    ...
    """
    # Load magnetic data
    magnetic = np.loadtxt(temp[0], skiprows = 1, usecols = [2, 7, 8, 9], dtype=float, converters={
        2: lambda s: float(s or np.nan),
        7: lambda s: float(s or np.nan),
        8: lambda s: float(s or np.nan),
        9: lambda s: float(s or np.nan)
    })

    magnetic[:,0] = dechour2decsol(magnetic[:,0], sol)
    ...
    # Load pressure data
    pressure = np.loadtxt(temp[1], delimiter=',', skiprows = 1, usecols=[3, 5], dtype=float, converters={
        3: time2decimal, 
    ...
    twins = np.loadtxt(temp[2], delimiter=',', skiprows = 1, usecols=[3, 5, 6, 7, 10, 13], dtype=float, converters={
        3: time2decimal,
        5: lambda s: float(s or np.nan),
        6: lambda s: float(s or np.nan),
        7: lambda s: float(s or np.nan),
    ...
    seis_rate_str = temp[4].split('_')[6]
    ...
    eng_data = np.loadtxt(temp[3], delimiter=',', skiprows=1, usecols=[3,45,48,49,50], dtype=float, converters={
        3: lambda s: (int(s.encode("utf-8").split(b':')[0]) / 24 + int(s.encode("utf-8").split(b':')[1]) / 1440 + float(s.encode("utf-8").split(b':')[2]) / 86400),
    ...
    eng_data[:,0] = dechour2decsol(eng_data[:,0], sol)


    #convert all ltst to fractional seconds
    
    magnetic[:,0] = LTSTfracSec(magnetic[:,0], magnetic_rate)
    pressure[:,0] = LTSTfracSec(pressure[:,0], pressure_rate)
    twins[:,0] = LTSTfracSec(twins[:,0], twins_rate)
    seis_data[:,0] = LTSTfracSec(seis_data[:,0], seis_rate)
    temperature[:,0] = LTSTfracSec(temperature[:,0], twins_rate)
...
# Define a function to handle the decimal hour format of LTST in ifg and eng files
def dechour2decsol(time, sol):
    time = np.array(time, dtype=float)
    solflag = np.zeros_like(time)
    
    wrap = np.diff(time) < 0
    wrap = np.insert(wrap, 0, False)

    if time[0] > 0 and time[0] < 10:
        wrap[0] = True
        
    solflag = solflag + np.cumsum(wrap)

    decsol = (sol - 1) + solflag + (time / 24.0)
...
def time2decimal(time):
    ...
    def convert_time(time_str):
        ...
        dayofyear, time_str = time_str.split(b' ', 1)
...
def LTSTfracSec(LTST, rate):
    onesec_dec = 1 / 86400

    change = np.diff(LTST, prepend=LTST[0]-1, append=LTST[-1]+1) != 0
    group_idx = np.flatnonzero(change)

    group_starts = group_idx[:-1]
    group_ends = group_idx[1:]
    ...
    for start, end, length in zip(group_starts, group_ends, group_lengths):
        if length == 1:
            continue
        if start == 0 and length < rate and len(group_starts) > 1:
            offsets[start:end] = np.arange(rate - length, rate) / rate
        elif end == n and length < rate and len(group_starts) > 1:
            offsets[start:end] = np.arange(length) / rate
        else:
            offsets[start:end] = np.arange(length) / rate

    return LTST + offsets * onesec_dec
...
def pres_residuals2(p, ydata, xdata):
    ...
    xmid = (min(xdata) + max(xdata)) / 2
    datarange = 50/86400
    idx_start = np.argmin(np.abs(xdata - xmid + datarange))
    idx_end = np.argmin(np.abs(xdata - xmid - datarange))
    x_res = xdata[idx_start:idx_end]
    y_res = ydata[idx_start:idx_end]

    # Basic validation
    if R <= 0:
        return np.ones_like(y_res) * 1e6

    peak_range = 20/86400 # allow to move by +-20 seconds

    if x0 < min(xdata) + peak_range or x0 > max(xdata) - peak_range:
        return np.ones_like(y_res) * 1e6
    if P0 > 0.00:
        return np.ones_like(y_res) * 1e6
    if R < 0.5/86400 or R > 200/86400:
        return np.ones_like(y_res) * 1e6

    # Compute the model predictions using the Lorentzian vortex pressure function
    ymodel = lv_pressure(x_res, 0, P0, x0, 0, R)
...
def pass_distance(i):
    ...
    tau_obs = key_data.loc[i,'time_FWHM'] * (87755.244/86400)
...
def Sol(SolNumber, plotflag):
    ...
    try:
        ...
        if paths:
            magnetic, magnetic_rate, pressure, pressure_rate, temperature, twins, twins_rate, seis, seis_rate, eng_data = data_extract(paths, SolNumber)
